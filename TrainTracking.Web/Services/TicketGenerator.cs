...
namespace TrainTracking.Web.Services
{
    public class TicketGenerator
    {
        ...
        public TicketGenerator(IWebHostEnvironment webHostEnvironment, IVirtualSegmentService virtualSegmentService)
        {
            _webHostEnvironment = webHostEnvironment;
            _virtualSegmentService = virtualSegmentService;
        }

        public async Task<byte[]> GenerateMultipleTicketsPdfAsync(List<Booking> bookings, string baseUrl, Dictionary<string, string> userNationalIds)
        {
            ...
            // We need to pre-fetch times because QuestPDF container logic doesn't support async well inside
            var ticketDataList = new List<TicketDisplayData>();
            foreach (var booking in bookings)
            {
                var departureTime = await _virtualSegmentService.CalculateDepartureTimeAsync(booking.Trip, booking.FromStation);
                var arrivalTime = await _virtualSegmentService.CalculateArrivalTimeAsync(booking.Trip, booking.ToStation);
                
                ticketDataList.Add(new TicketDisplayData
                {
                    Booking = booking,
                    DepartureTime = departureTime,
                    ArrivalTime = arrivalTime,
                    QrCodeImage = GenerateQrCode($"{baseUrl}/Bookings/TicketDetails/{booking.Id}"),
                    NationalId = userNationalIds.ContainsKey(booking.UserId) ? userNationalIds[booking.UserId] : ""
                });
            }

            var document = QuestPDF.Fluent.Document.Create(container =>
            {
                foreach (var data in ticketDataList)
                {
                        ...
                        page.Content().Element(content =>
                        {
                            content.Column(col =>
                            {
                                ...
                                col.Item().Padding(40).Row(mainRow =>
                                {
                                    mainRow.RelativeItem(2).Column(details =>
                                    {
                                        ...
                                        details.Item().Row(r =>
                                        {
                                            r.RelativeItem().Column(c =>
                                            {
                                                ...
                                                c.Item().Text(data.DepartureTime.ToString("yyyy-MM-dd")).FontSize(14).Bold().FontColor("#1a3c6e");
                                            });
                                            ...
                                            r.RelativeItem().AlignRight().Column(c =>
                                            {
                                                ...
                                                if (!string.IsNullOrEmpty(data.NationalId))
                                                {
                                                    c.Item().AlignRight().Text($"ID: {data.NationalId}").FontSize(9).FontColor(Colors.Grey.Darken2);
                                                }
                                            });
                                        });
                                        ...
                                        details.Item().Row(r =>
                                        {
                                            ...
                                            r.RelativeItem(2).Column(c =>
                                            {
                                                ...
                                                c.Item()
                                                 .Text(booking.ToStation?.Name ?? "Unknown")
                                                 .FontSize(16)
                                                 .Bold()
                                                 .FontColor("#e74c3c");

                                                c.Item()
                                                 .Text($"{data.ArrivalTime:HH:mm}")
                                                 .FontSize(12);
                                            });
                                             ...
                                             .Column(c =>
                                            {
                                                ...
                                                c.Item()
                                                 .AlignRight()
                                                 .Text(booking.FromStation?.Name ?? "Unknown")
                                                 .FontSize(16)
                                                 .Bold()
                                                 .FontColor("#e74c3c");
                                    ...
                                    {
                                        qrCol.Item().AlignCenter().PaddingTop(10).Width(100).Image(data.QrCodeImage);
        ...
        private class TicketDisplayData
        {
